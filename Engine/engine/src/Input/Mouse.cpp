...
namespace engine {
	...
	bool Mouse::init()
	{
		...
		hr = DirectInput8Create(
			EngineWindow()->getHInstance(), DIRECTINPUT_VERSION, IID_IDirectInput8, (LPVOID*)&device, NULL
		);
		if (FAILED(hr)) {
			return false;
		}
		...
		hr = lpDevice->SetCooperativeLevel(
			EngineWindow()->getHWnd(), DISCL_NONEXCLUSIVE | DISCL_FOREGROUND
		);
		if (FAILED(hr)) {
			return false;
		}
	...
	MousePosition Mouse::getMousePosition()
	{
		...
		// マウス座標をクライアント座標に変換
		ScreenToClient(EngineWindow()->getHWnd(), &mousePoint);
