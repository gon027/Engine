...
namespace engine {
	...
	DXGI_FORMAT convertToDXGI_FORMAT(TextureFormat _format) {
		switch (_format)
		{
	...
	TextureFormat convertToTextureFormat(DXGI_FORMAT _format) {
		switch (_format)
		{
	...
	ResourceDimension convertResourceDimention(D3D12_RESOURCE_DIMENSION _dimension) {
		switch (_dimension)
		{
	...
	D3D12_RESOURCE_DIMENSION convertD3D12ResourceDimension(ResourceDimension _dimension) {
		switch (_dimension)
		{
		case engine::ResourceDimension::Buffer:
			return D3D12_RESOURCE_DIMENSION_BUFFER;
		case engine::ResourceDimension::Texture1D:
			return D3D12_RESOURCE_DIMENSION_TEXTURE1D;
		case engine::ResourceDimension::Texture2D:
			return D3D12_RESOURCE_DIMENSION_TEXTURE2D;
		case engine::ResourceDimension::Texture3D:
			return D3D12_RESOURCE_DIMENSION_TEXTURE3D;
		case engine::ResourceDimension::Unknown:
	...
	bool Texture::loadTexture(const u8* _data, u64 _width, u64 _height, u64 _stride)
	{
		...
		resourceDimention = ResourceDimension::Texture2D;
